...
def main(gird_map, x_goal, y_goal, x_start, y_start):
    ...
    # get_current_pose = rospy.wait_for_message('/odom', Odometry, timeout=None)
    ...
    rospy.logwarn("Time taken to plan : {} Secs".format(str(round(end_time.secs - start_time.secs, 3))))
...
def redefined_goal(msg):
    main(temp, msg.pose.position.x, msg.pose.position.y)
...
if __name__ == '__main__':
    rospy.init_node("a_star_algo", anonymous=True)
    # temp = rospy.wait_for_message("/move_base/global_costmap/costmap", OccupancyGrid, timeout=None)
